// GENERATED by scripts/build.mjs from services/ai-svc/api/index.ts — do not edit directly.
...
function decodeUTF8(bytes) {
  let decoder2;
  return (decodeUTF8_ ?? (decoder2 = new globalThis.TextDecoder(), decodeUTF8_ = decoder2.decode.bind(decoder2)))(bytes);
}
...
import { constants as constants2, KeyObject as KeyObject4 } from "node:crypto";
...
var LOCALES = /* @__PURE__ */ new Set(["bn", "en", "bn-latn"]);
var _client = null;
function client() {
...
function redact(text) {
  return text.replace(/\+?880\s?1[0-9\s-]{8,12}/g, "[PHONE]").replace(/\b01[3-9][0-9]{8}\b/g, "[PHONE]").replace(/\b[0-9]{10,17}\b/g, "[ID]");
}
async function retrieve(db, ctx, q) {
...
async function sikhok(req, res, cors) {
...
${c.content}`).join("\n\n")}` : "\n\n(\u0995\u09CB\u09A8\u09CB \u09AA\u09BE\u09A0\u09CD\u09AF\u09AC\u0987 \u0985\u0982\u09B6 \u09AA\u09BE\u0993\u09AF\u09BC\u09BE \u09AF\u09BE\u09AF\u09BC\u09A8\u09BF \u2014 \u09B6\u09C1\u09A7\u09C1\u09AE\u09BE\u09A4\u09CD\u09B0 NCTB \u09AA\u09BE\u09A0\u09CD\u09AF\u0995\u09CD\u09B0\u09AE\u09C7\u09B0 \u09B8\u09BE\u09A7\u09BE\u09B0\u09A3 \u099C\u09CD\u099E\u09BE\u09A8 \u09AC\u09CD\u09AF\u09AC\u09B9\u09BE\u09B0 \u0995\u09B0\u09C1\u09A8 \u098F\u09AC\u0982 \u0985\u09A7\u09CD\u09AF\u09BE\u09AF\u09BC-\u09A8\u09BF\u09B0\u09CD\u09A6\u09BF\u09B7\u09CD\u099F \u09A6\u09BE\u09AC\u09BF \u098F\u09A1\u09BC\u09BF\u09AF\u09BC\u09C7 \u099A\u09B2\u09C1\u09A8\u0964)";
  const system = "You are SikhokAI, a teacher co-pilot for Bangladeshi schools. You generate NCTB-curriculum-compliant assessment material. Stay strictly within the named class level and subject. Answer in Bangla unless the locale is 'en'. Output well-structured Markdown.";
  const user = redact(
...
async function shikho(req, res, cors) {
  const claims = await authenticate(req);
  if (!await enforceIdentityRateLimit(res, cors, "ai", `${claims.tid}:${claims.sub}`)) return;
  const body = await readJson(req);
  const message2 = (body.message ?? "").trim().slice(0, 4e3);
  if (!message2) throw new HttpError(400, "message is required", "message_required");
  const locale = LOCALES.has(body.locale ?? "") ? body.locale : "bn";
...
async function handler(req, res) {
  ...
  const path5 = new URL(req.url ?? "/", "http://internal").pathname;
  const sub = path5.split("/").filter(Boolean).pop() ?? "";
