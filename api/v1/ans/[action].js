...
import { createHmac, createHash as createHash2 } from "node:crypto";
...
function requireServiceAuth(req, allowCron) {
  ...
  if (browserHeader) {
    ...
    throw new HttpError(
      403,
      "Service credentials may not be used from a browser",
...
async function inbound(req, res, cors) {
  ...
  const eventId = parsed.eventId ?? createHash2("sha256").update(raw).digest("hex").slice(0, 32);
