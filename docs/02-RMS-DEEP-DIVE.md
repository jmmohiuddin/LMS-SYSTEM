...
| `GET` | `/ops/document?type=routine_sheet&scope=…&id=…` | **As built (P9-9): the printable routine, on the school's letterhead. A seventh document type in R-5's machinery; imports `readTimetable` so print and screen share one authorisation and one read. Orientation follows content (portrait for section/teacher/room/student, landscape for the wide scopes) and pages split until no cell holds more than six lessons. Published only — draft, review and superseded answer `409 not_published`.** |
