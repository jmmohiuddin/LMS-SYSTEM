...
| Portals | R-3: principal dashboard, academic drill-down, class/section creation **and renaming** (`B-6`, closure pass), teacher assignment + replacement with history, bulk moves, rollover, users, guardian links, SMS settings, audit viewer. Guardian **unlinking** still has no screen and cannot until the data model can express an ended link (`B-7`) |
