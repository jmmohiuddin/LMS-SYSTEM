/**
 * POST /api/v1/finance/webhooks/bkash — see ../../src/webhook.ts for the
...
export default async function handler(req: IncomingMessage, res: ServerResponse): Promise<void> {
  ...
  try {
    ...
    const result = await p.process('bkash', rawBody, headers, sourceIp);
    json(res, result.status, result.body, CORS);
  } catch (err) {
    console.error('[finance/webhooks/bkash] unexpected error', err);
