...
export class RmsSolver {
    ...
    return this.db.withTenant(ctx, async (client) => {
      ...
      for (const d of sorted) {
        ...
        for (let i = 0; i < remaining; i++) {
          ...
          for (const preferUnusedDay of [true, false]) {
            for (const day of teachingDays) {
              ...
              for (const period of periods) {
                const tKey = `${d.teacherId}|${day}|${period.periodNo}`;
                const sKey = `${d.sectionId}|${day}|${period.periodNo}`;
          ...
          teacherBusy.add(`${d.teacherId}|${day}|${period.periodNo}`);
          sectionBusy.add(`${d.sectionId}|${day}|${period.periodNo}`);
          if (!sectionSubjectDays.has(ssKey)) sectionSubjectDays.set(ssKey, new Set());
  ...
  private async loadExistingSlots(client: pg.PoolClient, routineId: string) {
    const { rows } = await client.query<{
